...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  int n = 15;
  int n_inputs;
  double buffer_in[MAX_NUM_DATA][15];
  double buffer_out_mat[MAX_NUM_DATA][15];
  double buffer_out_btf[MAX_NUM_DATA][15];
  ...
  t_mat = clock();
  for (int i = 0; i < n_inputs; i++)
    gft_skeleton15_mat(buffer_in[i], buffer_out_mat[i]);
  t_mat = clock() - t_mat;
  ...
  t_btf = clock();
  for (int i = 0; i < n_inputs; i++)
    gft_skeleton15_btf(buffer_in[i], buffer_out_btf[i]);
  t_btf = clock() - t_btf;
