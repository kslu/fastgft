...
int main(int argc, char *argv[]) {
  ...
  int n = 80;
  int n_inputs;
  double buffer_in[BATCH_SIZE][80];
  double buffer_out_mat[BATCH_SIZE][80];
  double buffer_out_btf[BATCH_SIZE][80];
  ...
  for (int b = 0; b < n_batches; b++) {
    ...
    t_temp = clock();
    for (int i = 0; i < cur_batch_size; i++)
      gft_cycle80_mat(buffer_in[i], buffer_out_mat[i]);
    t_mat += clock() - t_temp;

    // butterfly GFT
    t_temp = clock();
    for (int i = 0; i < cur_batch_size; i++)
      gft_cycle80_btf(buffer_in[i], buffer_out_btf[i]);
    t_btf += clock() - t_temp;
